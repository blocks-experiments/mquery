...
export function validateDomManipulatePairs() {
    ...
    document.querySelectorAll('[dommanipulate]').forEach(btn => {
        ...
        toggleRefs.get(ref).buttons.push({ elem: btn, type: triggerType });
    });
    ...
    toggleRefs.forEach((data, ref) => {
        ...
        if (elements.length === 0 && buttons.length > 0) {
            errors.push(`❌ Orphaned [dommanipulate="${ref}"] buttons: No matching [refElemToggle="${ref}"] elements found`);
        }
        
        if (elements.length > 0 && buttons.length === 0) {
            errors.push(`❌ Orphaned [refElemToggle="${ref}"] elements: No matching [dommanipulate="${ref}"] buttons found`);
        }
